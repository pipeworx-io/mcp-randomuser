...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Random User');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    description: 'Generate random user profiles by gender ("male" or "female"). Returns names, contact info, photos, and demographics. Specify count for multiple profiles.',
    inputSchema: {
      type: 'object',
      properties: {
        gender: {
...
async function generateUsers(count: number, nationality?: string) {
  ...
  if (!res.ok) throw await httpError(res, 'Random User API error');
